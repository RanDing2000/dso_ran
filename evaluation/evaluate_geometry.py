...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_target_mesh", action="store_true", help="Save unique object IDs to target_mesh.txt")
    args = parser.parse_args()

    mesh_paths = sorted(glob(args.mesh_paths))
    
    # Extract unique object IDs and save to target_mesh.txt if requested
    if args.save_target_mesh:
        unique_object_ids = set()
        for mesh_path in mesh_paths:
            object_id = osp.basename(mesh_path).split("-")[0]
            unique_object_ids.add(object_id)
        
        with open("target_mesh.txt", "w") as f:
            for object_id in sorted(unique_object_ids):
                f.write(f"{object_id}\n")
        
        print(f"Saved {len(unique_object_ids)} unique object IDs to target_mesh.txt")
        print("Object IDs:", sorted(unique_object_ids))
