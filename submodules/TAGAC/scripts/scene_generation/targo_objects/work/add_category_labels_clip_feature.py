...
def generate_clip_features_for_scene(scene_name, target_category, model, device):
    """
    ...
    Returns:
        ...
        occluder_clip_features: CLIP features for occluders (512,)
    """
    ...
    target_clip_features = extract_clip_features(target_text_prompt, model, device)
    occluder_clip_features = extract_clip_features(occluder_text_prompt, model, device)
    
    print(f"Generated CLIP features for {scene_name}:")
    print(f"  Target category: {target_category}")
    print(f"  Target text prompt: '{target_text_prompt}'")
    print(f"  Target features shape: {target_clip_features.shape}")
    print(f"  Occluder features shape: {occluder_clip_features.shape}")
    
    return target_clip_features, occluder_clip_features

def save_clip_features_for_dataset(scene_name, target_clip_features, occluder_clip_features, output_dir):
    """
    Save CLIP features in the format expected by DatasetVoxel_PTV3_Clip.
    
    Args:
        scene_name: Name of the scene
        target_clip_features: CLIP features for target (512,)
        occluder_clip_features: CLIP features for occluders (512,)
        output_dir: Output directory for CLIP feature files
    """
    # Create output directory if it doesn't exist
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Save CLIP features in the format expected by DatasetVoxel_PTV3_Clip
    clip_feat_path = output_dir / f"{scene_name}.npz"
    
    # Expand features to match the expected format in the dataset
    # The dataset expects expanded features for each point
    target_clip_features_expanded = np.tile(target_clip_features, (512, 1))  # (512, 512)
    occluder_clip_features_expanded = np.tile(occluder_clip_features, (512, 1))  # (512, 512)
    
    # Save in the format expected by the dataset
    np.savez(
        clip_feat_path,
        targ_clip_features=target_clip_features_expanded,
        scene_clip_features_expanded=occluder_clip_features_expanded
    )
    
    print(f"Saved CLIP features to {clip_feat_path}")
    print(f"  Target features expanded shape: {target_clip_features_expanded.shape}")
    print(f"  Scene features expanded shape: {occluder_clip_features_expanded.shape}")
...
def main():
    ...
    parser.add_argument("--clip_feat_dir", type=str,
                       default="data_scenes/targo_dataset/scene_ptv3_clip_feat",
                       help="Directory to save CLIP feature files")
    parser.add_argument("--max_files", type=int, default=None,
                       help="Maximum number of files to process (for testing)")
    parser.add_argument("--generate_clip_features",  default=True,
                       help="Generate and save CLIP features for each scene")
    ...
    clip_feat_dir = Path(args.clip_feat_dir)
    ...
    # Load existing category scene dictionary
    category_scene_dict_path = "data_scenes/targo_dataset/category_scene_dict.json"
    print(f"Loading existing category scene dictionary from {category_scene_dict_path}")
    with open(category_scene_dict_path, 'r') as f:
        category_scene_dict = json.load(f)
    
    print(f"Loaded {len(category_scene_dict)} scene-category mappings")
    ...
    processed_clip_features = 0
    missing_category_files = []
    
    # Process each npz file using loaded category dictionary
    for npz_file in tqdm(npz_files, desc="Processing npz files"):
        ...
        # Check if scene has category in the loaded dictionary
        if scene_name not in category_scene_dict:
            missing_category_files.append(scene_name)
            continue
            
        target_category = category_scene_dict[scene_name]
        
        # Generate and save CLIP features if requested
        if args.generate_clip_features:
            try:
                target_clip_features, occluder_clip_features = generate_clip_features_for_scene(
                    scene_name, target_category, model, device
                )
                save_clip_features_for_dataset(
                    scene_name, target_clip_features, occluder_clip_features, clip_feat_dir
                )
                processed_clip_features += 1
            except Exception as e:
                print(f"Error generating CLIP features for {scene_name}: {e}")
                import traceback
                traceback.print_exc()
    
    # Save files without category to tmp/missing_category_files.txt
    if missing_category_files:
        missing_category_file_path = tmp_dir / "missing_category_files.txt"
        with open(missing_category_file_path, 'w') as f:
            for scene_name in missing_category_files:
                f.write(f"{scene_name}\n")
        print(f"Found {len(missing_category_files)} files without category mapping, saved to {missing_category_file_path}")
    else:
        print("All processed files have category mappings")
    
    # Print summary
    if args.generate_clip_features:
        print(f"\nCLIP Feature Generation Summary:")
        print(f"  Total scenes with category mapping: {len(category_scene_dict)}")
        print(f"  CLIP features generated: {processed_clip_features}")
        print(f"  CLIP features saved to: {clip_feat_dir}")
        print(f"  Files without category mapping: {len(missing_category_files)}")
    else:
        print(f"\nCategory Processing Summary:")
        print(f"  Total scenes with category mapping: {len(category_scene_dict)}")
        print(f"  Files without category mapping: {len(missing_category_files)}")
        print(f"  To generate CLIP features, run with --generate_clip_features flag")
