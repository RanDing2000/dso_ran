...
from vgn.utils.implicit import get_mesh_pose_list_from_world,get_mesh_pose_dict_from_world 
import MinkowskiEngine as ME

## TODO add noise to the tsdf and point cloud
## TODO, genderate depth pc
## TODO, contains plane in the grid
...
def save_mask_visualization(mask, path, cmap='gray'):
    """
    Saves a visualization of a binary mask to a specified file path.

    Parameters:
    - mask (np.array): A 2D NumPy array representing the mask, where masked regions are non-zero.
    - path (str): The file path where the image will be saved.
    - cmap (str, optional): The colormap used for visualization. Defaults to 'gray'.
    ...
    # Use plt.imsave to save the mask directly to a file
    plt.imsave(path, mask, cmap=cmap)

def create_occluder_mask(segmentation_image, occluder_ids):
    """
    Creates a mask for the occluders in the scene. Returns an empty mask if occluder_ids is None
    """
    if occluder_ids is None:
        return np.zeros_like(segmentation_image, dtype=int)

    mask = np.isin(segmentation_image, occluder_ids).astype(int)
    return mask
...
def main(args, s):
    ...
    if args.save_scene:
        # path = f'{args.root}/mesh_pose_list'
...
def generate_scenes(sim,seed=0):
    ...
    # save_point_cloud_as_ply(np.asarray(pc_c.points), 'pc_c.ply')
    if pc_c.is_empty():
        return
    
    scene_id = uuid.uuid4().hex
    if args.save_scene:
        ## TODO: mesh_pose_list is True?
    ...
    for target_id in body_ids:
        ...
        # pc_targ_side_s = reconstruct_40_pc(sim, depth_side_s * mask_targ_side_s, extr_side_s) # point cloud of the single scene
        # pc_scene_side_s = reconstruct_40_pc(sim, depth_side_s, extr_side_s) # point cloud of the single scene
        # grid_targ_side_s = reconstruct_40_grid(sim, depth_side_s * mask_targ_side_s, extr_side_s) # grid of the single scene
        # grid_scene_side_s = reconstruct_40_grid(sim, depth_side_s, extr_side_s) # grid of the single scene
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root",type=Path, default= "/home/ran.ding/projects/TARGO/data/giga/demo/data_set_theory_01")
    # parser.add_argument("--root",type=Path, default= "/home/ran.ding/projects/TARGO/data/nips_data_version1")
