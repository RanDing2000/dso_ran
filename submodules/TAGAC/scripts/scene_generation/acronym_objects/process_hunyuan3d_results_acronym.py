#!/usr/bin/env python3
"""
Script to process Hunyuan3D reconstruction results for ACRONYM dataset.
...
def process_hunyuan3d_results_acronym(hunyuan_results_path, max_scenes=None, dry_run=False):
    """
    Process Hunyuan3D reconstruction results for ACRONYM dataset to generate point clouds and TSDF.
    
    Args:
        hunyuan_results_path: Path to Hunyuan3D results directory for ACRONYM
    ...
    for scene_dir in tqdm(scene_dirs, desc="Processing ACRONYM Hunyuan3D results"):
        ...
        try:
            ...
            try:
                ...
                # Use newer methods to clean up mesh
    ...
    if stats['error_dirs']:
        ...
        save_error_dirs_to_txt(stats['error_dirs'], script_dir, "acronym_hunyuan3d")

def save_error_dirs_to_txt(error_dirs, script_dir, dataset_name="acronym_hunyuan3d"):
    """
...
def main():
    parser = argparse.ArgumentParser(description='Process ACRONYM Hunyuan3D reconstruction results to generate point clouds and TSDF')
    parser.add_argument('--hunyuan_path', type=str, 
                       default='/home/ran.ding/projects/Gen3DSR/hunyuan_results/acronym/slight',
                       help='Path to ACRONYM Hunyuan3D results directory')
    ...
    print("ACRONYM Hunyuan3D Results Processor")
    print("="*50)
    ...
    if not os.path.exists(args.hunyuan_path):
        print(f"Error: ACRONYM Hunyuan3D results path does not exist: {args.hunyuan_path}")
        return
    
    process_hunyuan3d_results_acronym(args.hunyuan_path, args.max_scenes, args.dry_run)
