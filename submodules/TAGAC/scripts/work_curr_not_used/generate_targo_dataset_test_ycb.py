...
## [] write the json file of scene_id and occlusion level
## write all the things like the generate training dataset
...
def generate_scenes(sim):
    ...
    for target_id in body_ids:
        ...
        if occ_level_c < 0.1:
            ...
            current_count = occ_level_dict_count["0.1-0.2"]
            if current_count >= MAX_BIN_COUNT:
                sim.world.remove_body(target_body)
                continue
            else:
                if process_and_store_scene_data(sim, scene_id, target_id, noisy_depth_side_c, seg_side_c, extr_side_c, args, occ_level_c) != None:
                    occ_level_dict_count["0.1-0.2"] += 1
                    occ_level_scene_dict[scene_id + '_c_' + str(target_id)] = occ_level_c
                    write_test_set_point_cloud(args.root, scene_id + f"_c_{target_id}", mesh_clutter_pose_dict, target_body.name, name="mesh_pose_dict")
        elif 0.2 <= occ_level_c < 0.3:
            current_count = occ_level_dict_count["0.2-0.3"]
            if current_count >= MAX_BIN_COUNT:
                sim.world.remove_body(target_body)
                continue
            else:
                if process_and_store_scene_data(sim, scene_id, target_id, noisy_depth_side_c, seg_side_c, extr_side_c, args, occ_level_c) != None:
                    occ_level_dict_count["0.2-0.3"] += 1
                    occ_level_scene_dict[scene_id + '_c_' + str(target_id)] = occ_level_c
                    write_test_set_point_cloud(args.root, scene_id + f"_c_{target_id}", mesh_clutter_pose_dict, target_body.name,  name="mesh_pose_dict")

        elif 0.3 <= occ_level_c < 0.4:
            current_count = occ_level_dict_count["0.3-0.4"]
            if current_count >= MAX_BIN_COUNT:
                sim.world.remove_body(target_body)
                continue
            else:
                if process_and_store_scene_data(sim, scene_id, target_id, noisy_depth_side_c, seg_side_c, extr_side_c, args, occ_level_c) != None:
                    occ_level_dict_count["0.3-0.4"] += 1
                    occ_level_scene_dict[scene_id + '_c_' + str(target_id)] = occ_level_c
                    write_test_set_point_cloud(args.root, scene_id + f"_c_{target_id}", mesh_clutter_pose_dict, target_body.name, name="mesh_pose_dict")

        elif 0.4 <= occ_level_c < 0.5:
            current_count = occ_level_dict_count["0.4-0.5"]
            if current_count >= MAX_BIN_COUNT:
                sim.world.remove_body(target_body)
                continue
            else:
                if  process_and_store_scene_data(sim, scene_id, target_id, noisy_depth_side_c, seg_side_c, extr_side_c, args, occ_level_c) != None:
                    occ_level_dict_count["0.4-0.5"] += 1
                    occ_level_scene_dict[scene_id + '_c_' + str(target_id)] = occ_level_c
                    write_test_set_point_cloud(args.root, scene_id + f"_c_{target_id}", mesh_clutter_pose_dict, target_body.name, name="mesh_pose_dict")

        elif 0.5 <= occ_level_c < 0.6:
            current_count = occ_level_dict_count["0.5-0.6"]
            if current_count >= MAX_BIN_COUNT:
                sim.world.remove_body(target_body)
                continue
            else:
                if process_and_store_scene_data(sim, scene_id, target_id, noisy_depth_side_c, seg_side_c, extr_side_c, args, occ_level_c) != None:
                    occ_level_dict_count["0.5-0.6"] += 1
                    occ_level_scene_dict[scene_id + '_c_' + str(target_id)] = occ_level_c
                    write_test_set_point_cloud(args.root, scene_id + f"_c_{target_id}", mesh_clutter_pose_dict, target_body.name, name="mesh_pose_dict")

        elif 0.6 <= occ_level_c < 0.7:
            current_count = occ_level_dict_count["0.6-0.7"]
            if current_count >= MAX_BIN_COUNT:
                sim.world.remove_body(target_body)
                continue
            else:
                if process_and_store_scene_data(sim, scene_id, target_id, noisy_depth_side_c, seg_side_c, extr_side_c, args, occ_level_c) != None:
                    occ_level_dict_count["0.6-0.7"] += 1
                    occ_level_scene_dict[scene_id + '_c_' + str(target_id)] = occ_level_c
                    write_test_set_point_cloud(args.root, scene_id + f"_c_{target_id}", mesh_clutter_pose_dict, target_body.name, name="mesh_pose_dict")

        elif 0.7 <= occ_level_c < 0.8:
            current_count = occ_level_dict_count["0.7-0.8"]
            if current_count >= MAX_BIN_COUNT:
                sim.world.remove_body(target_body)
                continue
            else:
                if process_and_store_scene_data(sim, scene_id, target_id, noisy_depth_side_c, seg_side_c, extr_side_c, args, occ_level_c) != None:
                    occ_level_dict_count["0.7-0.8"] += 1
                    occ_level_scene_dict[scene_id + '_c_' + str(target_id)] = occ_level_c
                    write_test_set_point_cloud(args.root, scene_id + f"_c_{target_id}", mesh_clutter_pose_dict, target_body.name, name="mesh_pose_dict")

        elif 0.8 <= occ_level_c < 0.9:
            current_count = occ_level_dict_count["0.8-0.9"]
            if current_count >= MAX_BIN_COUNT:
                sim.world.remove_body(target_body)
                continue
            else:
                if process_and_store_scene_data(sim, scene_id, target_id, noisy_depth_side_c, seg_side_c, extr_side_c, args, occ_level_c) != None:
                    occ_level_dict_count["0.8-0.9"] += 1
                    occ_level_scene_dict[scene_id + '_c_' + str(target_id)] = occ_level_c
                    write_test_set_point_cloud(args.root, scene_id + f"_c_{target_id}", mesh_clutter_pose_dict, target_body.name, name="mesh_pose_dict")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root",type=Path, default= 'data_scenes/maniskill-ycb-v2')
