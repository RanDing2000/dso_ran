...
from utils_giga import record_occ_level_count, record_occ_level_success, cal_occ_level_sr, save_point_cloud_as_ply, generate_and_transform_grasp_meshes
import trimesh
...
def run(
    ...
    # complete_target_ablation = ""
    complete_targ_ablation = "",
):
    ...
    # for num_id, curr_mesh_pose_list in enumerate(os.listdir(test_scenes)):
        ...
        if type in ('vgn', 'giga_aff', 'giga', 'giga_hr'):
            ...
            if fusion_type in ('CNN_concat','CNN_add', 'MLP_fusion'):
                ...
                if shape_completion:
                    ...
                    state = argparse.Namespace(tsdf=tsdf,scene_no_targ_pc = scene_no_targ_pc, targ_pc = targ_pc, occ_level=occ_level,type = type, fusion_type = fusion_type, complete_targ_ablation = complete_targ_ablation)
                else:
        ...
        scene_pc = (scene_pc + 0.5) * 0.3
        generate_and_transform_grasp_meshes(grasp, scene_pc,'/usr/stud/dira/GraspInClutter/grasping/demo_targo' )
        # grasp.width *= 1.5
        grasp.width = sim.gripper.max_opening_width
        label, plan_failure, visual_failure = sim.execute_grasp(grasp, allow_contact=True, tgt_id = tgt_id, force_targ = True)
        if plan_failure ==1:
    ...
    print("done")
    ## write visual_failure_count and plan_failure_count
