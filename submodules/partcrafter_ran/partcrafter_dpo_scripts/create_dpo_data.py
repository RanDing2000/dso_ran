...
class DPODataCreator:
    ...
    def run_inference(self, 
                     ...
                     rmbg_net: Any = None,
                     dtype: torch.dtype = torch.float16,
                     device: str = "cuda") -> Tuple[List[trimesh.Trimesh], Image.Image, torch.Tensor, torch.Tensor]:
        """
        ...
        processed_image = self.pipeline.feature_extractor_dinov2(img_pil, return_tensors="pt").pixel_values.to(device)
        with torch.no_grad():
        ...
        part_latents = outputs.latents
 
        
        return meshes, img_pil, cond_features, part_latents
    ...
    def create_single_case_dpo_data(self, 
        ...
        try:
            ...
            cond_features_path = os.path.join(case_output_dir, "cond.pt")
            torch.save(cond_features.cpu().to(torch.bfloat16), cond_features_path)
            print(f"Saved cond features: {cond_features_path}")
            ...
            for i in range(config['num_parts']):
                latent_part_path = os.path.join(case_output_dir, f"latent_sample_{i:03d}.pt")
                if latent_features is not None and i < latent_features.shape[0]:
                    ...
                    print(f"Saved latent features for part {i}: {latent_part_path}")
                else:
                    ...
                    print(f"Saved dummy latent for part {i}: {latent_part_path}")
...
def main():
    ...
    parser.add_argument('--config_path', type=str, 
                       default='/home/ran.ding/messy-kitchen/PartCrafter/data/preprocessed_data_messy_kitchen/messy_kitchen_configs.json',
