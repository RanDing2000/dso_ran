...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if accelerator.is_main_process:
        # Create a clean config for wandb without runtime objects
        ...
        # Whitelist only training hyperparameters
        whitelist = {
            'exp_name', 'output_dir', 'dataset_dir', 'dataset_kwargs', 'category',
            ...
            'use_sft', 'use_dro', 'use_dpo'
        }
        wandb_config = {k: v for k, v in clean_config.items() if k in whitelist}
        
        accelerator.init_trackers(project_name="dso", config=wandb_config, init_kwargs={"wandb": {"name": exp_name}})
    ...
    while global_step < max_train_steps:
        for batch in train_loader:
            ...
            t = sample_flow_matching_t_for_training(
                ...
                bsz=bsz,
            ).to(device=batch['cond'].device, dtype=batch['cond'].dtype)
                ...
                with accelerator.autocast():
                    ...
                    if use_dpo:
                        loss = forward_dpo_loss(
                            model=model,
                            ref_model=ref_model,
                            x0_win=batch['model_win_sparse_x0'],    # model_win_sparse_x0: [bsz, 8, 16, 16, 16]
                            x0_loss=batch['model_loss_sparse_x0'],    # model_loss_sparse_x0: [bsz, 8, 16, 16, 16]
                            t=t,
                            cond=batch['cond'],                  # cond: [bsz, 1374, 1024]
                            beta=dpo_beta,
